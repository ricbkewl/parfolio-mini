...
function renderHub(){
  ...
  if(title){const eye=title.querySelector('.eyebrow'),h=title.querySelector('h1'),p=title.querySelector('p:last-child');if(eye&&eye.textContent!=='My ParFolio')eye.textContent='My ParFolio';if(h&&h.textContent!=='Your golf identity, in one place.')h.textContent='Your golf identity, in one place.';const copy='Keep your player details, Nimiq wallet, clubs and signed rounds together. Your connected Nimiq wallet is your Mini sign-in credential for this competition build.';if(p&&p.textContent!==copy)p.textContent=copy}
...
function simplifyNav(){
  ...
  drawer.querySelectorAll('.pf-nav-item').forEach(btn=>{const p=btn.dataset.page;if(p==='wallet'&&btn.textContent.trim()!=='My ParFolio')btn.innerHTML='<i>◈</i>My ParFolio';if((p==='rounds'||p==='clubs')&&btn.style.display!=='none')btn.style.display='none'})
}

function run(){renderHub();simplifyNav()}
let queued=false
const observer=new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;runSafely()})})
function runSafely(){observer.disconnect();try{run()}finally{observer.observe(document.body,{childList:true,subtree:true})}}
window.addEventListener('parfolio:profile-updated',()=>setTimeout(runSafely,20))
runSafely()
