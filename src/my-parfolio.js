...
function connectedWallet(){const text=document.querySelector('#walletMessage')?.textContent||'';const m=text.match(/Connected as\s+(.+?)\.\s*Tap/i);return m?.[1]?.trim()||''}
...
let tries=0
const bootTimer=setInterval(()=>{tries++;if(renderHub()||tries>50)clearInterval(bootTimer)},120)
window.addEventListener('parfolio:profile-updated',()=>setTimeout(()=>renderHub(true),30))
window.addEventListener('parfolio:wallet-updated',()=>setTimeout(()=>renderHub(true),30))
document.addEventListener('click',e=>{const page=e.target.closest?.('[data-page="wallet"]');if(page)setTimeout(()=>renderHub(),0)},true)
renderHub()
