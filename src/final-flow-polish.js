...
function installStyles(){
  ...
  @media(max-width:720px){.pf-my-identity-head{padding:16px;border-radius:20px}.pf-my-identity-head h1{font-size:2.25rem}.pf-play-path{margin-left:0;margin-right:0}}
  `
...
function polishPlay(){
  const page=document.querySelector('.pf-app-page[data-page="play"]'),verify=page?.querySelector('#verifyRound');if(!verify)return
...
function polishMy(){
  ...
  const title=page.querySelector('.pf-page-title');if(title&&title.style.display!=='none')title.style.display='none'
...
function polishNav(){
  ...
  drawer.querySelectorAll('.pf-nav-item').forEach(btn=>{
    ...
    if((btn.dataset.page==='rounds'||btn.dataset.page==='clubs')&&btn.style.display!=='none')btn.style.display='none'
  })
}
...
const observer=new MutationObserver(()=>{
  if(queued)return
  queued=true
  requestAnimationFrame(()=>{queued=false;runSafely()})
})
function runSafely(){
  observer.disconnect()
